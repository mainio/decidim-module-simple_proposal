...
describe Decidim::ScopesHelper do
  ...
  describe "#scopes_picker_field" do
    subject do
      Nokogiri::HTML(helper.scopes_picker_field(form, :decidim_scope_id, root:))
    end

    let(:target) { subject.css("select") }

    let(:root_scope) { create(:scope, organization:) }
    let(:scopes) { create_list(:scope, 4, parent: root_scope) }
    let(:another_root_scope) { create(:scope, organization:) }
    ...
    context "with root scope" do
      ...
      it "renders only the scopes under the root scope" do
        expect(target).to have_no_css("option", text: translated(root_scope.name))
        scopes.each do |scope|
          expect(target).to have_css("option", text: translated(scope.name))
        end
        expect(target).to have_no_css("option", text: translated(another_root_scope.name))
        other_scopes.each do |scope|
          expect(target).to have_no_css("option", text: translated(scope.name))
        end
      end
